...
- เข้าถึง **SchoolOS Public API** ที่ `http://192.168.200.56:3002` ได้ (ครู/นักเรียน/ล็อกอิน — ดู API-KEYS.md)
...
| ตัวแปร | คำอธิบาย |
|---|---|
| `ARENA_DB_HOST` / `ARENA_DB_PORT` / `ARENA_DB_USER` / `ARENA_DB_PASSWORD` / `ARENA_DB_NAME` | ค่า DB ที่ docker compose เอาไปประกอบเป็น `DATABASE_URL` ของ app (host = `postgres-core`) |
| `DATABASE_URL` | ใช้ตอน dev บน host: `postgres://arena:รหัส@localhost:5432/arena` (ตอนรัน compose ถูก override) |
| `JWT_SECRET` | สุ่มค่ายาว เช่น `openssl rand -hex 32` — **ห้ามใช้ค่าตัวอย่าง** |
| `SCHOOLOS_API_BASE` | `http://192.168.200.56:3002` (ไม่ต้องมี `/api/public/v1`) |
| `SCHOOLOS_API_KEY` | key เดียวหลาย scope: `students:read`, `teachers:read`, `auth:students`, `auth:teachers` |
| `BASE_PATH` | subpath ที่เสิร์ฟ — เว้นว่างสำหรับ dev, prod ตั้ง `/arena` (ดูข้อ 7) |

## 4. สร้าง Database

**Docker** — arena **ไม่สร้าง container db เอง** แล้ว แต่ไปเกาะ `postgres-core` ที่มีอยู่
(สแตกกลาง 1 Postgres ต่อหลายแอป — เหมือน `schoolos-users-app`) ต้องมี database + role `arena` อยู่ก่อน:
```bash
docker exec -it postgres-core psql -U admin -d postgres \
  -c "CREATE ROLE arena LOGIN PASSWORD 'รหัส';" \
  -c "CREATE DATABASE arena OWNER arena;"
```
จากนั้น `docker compose up -d --build` จะต่อผ่าน network `school-net` ที่ `postgres-core:5432`
(บน host เข้าถึงตัวเดียวกันได้ที่ `localhost:5432`)
...
|---|---|---|
| **student** | รหัสนักเรียน + รหัสผ่าน SchoolOS (`/auth/verify`) | ลงทะเบียน/ถอนตัว/ดูรายการของตน |
| **teacher** | รหัสครู + รหัสผ่าน SchoolOS (`/auth/verify`) | สร้าง/แก้รายการที่ตนสร้าง, เพิ่ม-ลดผู้เข้าแข่ง |
