...
export async function POST(req: Request) {
  return handle(async () => {
    ...
    if (!comp.isPublished) return fail("ต้องประกาศผลรายการนี้ก่อนจึงจะออกเกียรติบัตรได้");
    ...
    if (!event) return fail("รายการนี้ยังไม่ถูกจัดเข้างานเกียรติบัตร กรุณาแจ้งผู้ดูแลระบบ");
    if (event.status === "draft")
      return fail("งานเกียรติบัตรนี้ยังไม่เผยแพร่ กรุณารอผู้ดูแลระบบตั้งค่าให้เสร็จ");
    ...
    if (!targets.length) return fail("ไม่มีผู้เข้าแข่งขันให้ออกเกียรติบัตร");
