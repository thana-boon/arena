import { z } from "zod";
import { db } from "@/db";
import { competitions, criteria, entries, entryMembers, scores } from "@/db/schema";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  return handle(async () => {
    ...
    if (!canScore(s, comp.createdBy, groupNos)) return fail("บันทึกคะแนนได้เฉพาะรายการในหมวดของท่าน", 403);
