...
export const competitions = pgTable(
  "competitions",
  {
    ...
    /**
     * เช็คชื่อผู้เข้าร่วมของรายการที่ "ไม่มีการแข่งขัน" ครั้งล่าสุดเมื่อไหร่ (null = ยังไม่เคยเช็คชื่อ)
     *
     * รายการแบบนี้ไม่มีคะแนนให้บันทึก มีแค่ "มาร่วมกิจกรรมจริงไหม" — ครูติ๊กที่หน้าเช็คชื่อ
     * (/teacher/scoring/[id]) แล้วคนที่ไม่ได้ติ๊กจะกลายเป็น absent = ไม่ได้เกียรติบัตร
     *
     * ต้องมีคอลัมน์นี้เพราะ absent = false ตอบไม่ได้ว่า "เช็คแล้วว่ามา" หรือ "ยังไม่เคยเช็ค"
     * ถ้าไม่แยกสองอย่างนี้ ครูที่ลืมเช็คชื่อจะออกใบให้ทุกคนที่ลงทะเบียนไว้โดยไม่รู้ตัว
     * → ขาออกเกียรติบัตรจึงบล็อกไว้จนกว่าจะเช็คชื่อครั้งแรก (ดู domain.certIssueGate)
     */
    attendanceCheckedAt: timestamp("attendance_checked_at"),
    attendanceCheckedBy: varchar("attendance_checked_by", { length: 64 }),
...
export const entryMembers = pgTable(
  "entry_members",
  {
     ...
     *
     * ⚠ รายการที่ "ไม่มีการแข่งขัน" (competitions.no_contest) กลับด้านกัน: เริ่มที่ absent = true
     * ตั้งแต่ลงทะเบียน แปลว่า "ยังไม่ได้เช็คชื่อ = ถือว่าไม่มาร่วม" ต้องติ๊ก "เข้าร่วม" ที่หน้าเช็คชื่อ
     * จึงจะได้ใบ (ไม่ใช่ทุกคนได้โดยปริยาย — ดู competitions.attendance_checked_at)
     *
